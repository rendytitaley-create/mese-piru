...
const PIRUApp = () => {
  ...
  const handleSubmitReport = async (e) => {
    ...
    try {
      if (isEditing && currentReportId) {
        ...
        if (['pimpinan', 'admin'].includes(user.role) && newReport.targetUser) {
  ...
  const dashboardStats = useMemo(() => {
    ...
    return { myTotal: myReports.length };
  ...
  if (!user) return (
    <div className="h-screen bg-slate-900 flex items-center justify-center p-4 italic">
      <div className="bg-white w-full max-w-md rounded-[2.5rem] p-12 shadow-2xl text-center">
        ...
        <form onSubmit={handleLogin} className="space-y-4 text-left not-italic">
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex font-sans overflow-hidden text-slate-800 italic">
      {/* SIDEBAR */}
      <div className="w-72 bg-white border-r p-8 flex flex-col hidden md:flex not-italic">
        <div className="flex items-center gap-4 mb-14 px-2 italic">
          ...
          <div><h2 className="font-black text-2xl uppercase tracking-tighter italic">PIRU</h2><p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-1 italic">BPS SBB</p></div>
        </div>
        <nav className="flex-1 space-y-3">
          <button onClick={() => setActiveTab('dashboard')} className={`w-full flex items-center gap-4 p-5 rounded-3xl font-black text-xs uppercase ${activeTab === 'dashboard' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-400 hover:bg-slate-50'}`}><BarChart3 size={20}/> Dashboard</button>
          <button onClick={() => setActiveTab('laporan')} className={`w-full flex items-center gap-4 p-5 rounded-3xl font-black text-xs uppercase ${activeTab === 'laporan' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-400 hover:bg-slate-50'}`}><FileText size={20}/> Capaian Kerja</button>
          {user.role === 'admin' && (<button onClick={() => setActiveTab('users')} className={`w-full flex items-center gap-4 p-5 rounded-3xl font-black text-xs uppercase ${activeTab === 'users' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-400 hover:bg-slate-50'}`}><Users size={20}/> Data Pegawai</button>)}
        </nav>
      ...
      <main className="flex-1 p-10 overflow-y-auto italic">
        {/* HEADER SIMETRIS */}
        <header className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-8 mb-10 italic">
          ...
          <div className="flex flex-wrap items-center gap-3 not-italic xl:justify-end">
             {['admin', 'pimpinan', 'ketua'].includes(user.role) && activeTab === 'laporan' && (
                <select className="p-3 bg-white border border-slate-200 rounded-xl font-black text-[10px] text-slate-600 outline-none" value={filterStaffName} onChange={e => setFilterStaffName(e.target.value)}>
                  <option value="Semua">Semua Pegawai</option>
                  {users.filter(u => !['admin','pimpinan'].includes(u.role)).map(u => <option key={u.firestoreId} value={u.name}>{u.name}</option>)}
                </select>
              )}
            <select className="bg-white border border-slate-200 rounded-xl px-4 py-3 font-black text-[10px] text-slate-600 outline-none" value={selectedMonth} onChange={e => setSelectedMonth(Number(e.target.value))}>
        ...
        {activeTab === 'users' ? (
          <div className="bg-white rounded-[3rem] shadow-sm border overflow-hidden p-10 italic">
            <table className="w-full text-left italic">
              ...
              <tbody>{users.map(u => (<tr key={u.firestoreId} className="border-b hover:bg-slate-50 italic"><td className="p-8 font-black text-slate-800 uppercase">{u.name}</td><td className="p-8 font-bold text-slate-500 text-xs uppercase">{u.role}</td><td className="p-8 text-center"><button onClick={() => deleteDoc(doc(db, "users", u.firestoreId))} className="p-4 bg-red-50 text-red-400 rounded-2xl hover:bg-red-500 hover:text-white transition-all"><Trash2 size={20}/></button></td></tr>))}</tbody>
            </table>
          </div>
        ) : (
              ...
              <table className="w-full text-left italic">
                ...
                <tbody className="divide-y divide-slate-50 italic">
                  {currentFilteredReports.map(r => (
                    <tr key={r.id} className="hover:bg-slate-50/50 italic">
                      ...
                      <td className="p-8 text-center">
                        <div className="flex justify-center gap-2">
                          ...
                          {['ketua', 'admin'].includes(user.role) && r.userId !== user.username && r.status !== 'selesai' && (
                            <button onClick={() => submitGrade(r.id, 'ketua')} className="bg-amber-400 text-white px-5 py-2.5 rounded-2xl text-[10px] font-black uppercase italic">Ketua</button>
                          )}
                          {['pimpinan', 'admin'].includes(user.role) && r.userId !== user.username && (
                            <button onClick={() => submitGrade(r.id, 'pimpinan')} className="bg-indigo-600 text-white px-5 py-2.5 rounded-2xl text-[10px] font-black uppercase italic">Pimpinan</button>
                          )}
                        </div>
                      </td>
                    </tr>
      ...
      {/* MODAL USER */}
      {showUserModal && (
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-xl flex items-center justify-center p-4 z-50 italic">
          <form onSubmit={handleAddUser} className="bg-white w-full max-w-xl rounded-[3.5rem] p-16 shadow-2xl relative">
            ...
            <div className="space-y-5">
                ...
                <div className="grid grid-cols-2 gap-5">
                    ...
                    <input required type="text" placeholder="Password" className="w-full p-6 bg-slate-50 rounded-3xl outline-none font-black text-slate-700 border border-slate-100" onChange={e => setNewUser({...newUser, password: e.target.value})} />
                </div>
