...
const PIRUApp = () => {
  ...
  const handleAddUser = async (e) => {
    e.preventDefault();
    try {
      // POKOK MASALAH: Memastikan role bersih dari gaya tulisan agar sistem mengenali hak akses
      const roleUtama = newUser.role.split(' ')[0]; 
  ...
  const handleSubmitReport = async (e) => {
    ...
    try {
      if (isEditing && currentReportId) {
        ...
        // Mendeteksi role tanpa gangguan gaya miring
        const currentRole = user.role.split(' ')[0];
        if (['pimpinan', 'admin'].includes(currentRole) && newReport.targetUser) {
  ...
  const currentFilteredReports = useMemo(() => {
    ...
    const currentRole = user?.role?.split(' ')[0];
    if (currentRole === 'pegawai') res = res.filter(r => r.userId === user.username);
    if (['pimpinan', 'admin', 'ketua'].includes(currentRole) && filterStaffName !== 'Semua') {
  ...
  const exportToExcel = async () => {
    if (filterStaffName === 'Semua' && user.role.split(' ')[0] !== 'pegawai') {
        alert("Pilih satu nama pegawai terlebih dahulu.");
        return;
    }
    const targetStaff = user.role.split(' ')[0] === 'pegawai' ? user : users.find(u => u.name === filterStaffName);
  ...
  if (!user) return (
    <div className="h-screen bg-slate-900 flex items-center justify-center p-4">
      <div className="bg-white w-full max-w-md rounded-[2.5rem] p-12 shadow-2xl text-center font-sans">
        ...
        <h1 className="text-4xl font-black mb-1 tracking-tighter text-slate-800 uppercase">PIRU</h1>
        <p className="text-[10px] font-black text-indigo-600 uppercase tracking-widest mb-10">BPS KABUPATEN SERAM BAGIAN BARAT</p>
    ...
    <div className="min-h-screen bg-slate-50 flex font-sans overflow-hidden text-slate-800">
      <div className="w-72 bg-white border-r p-8 flex flex-col hidden md:flex font-sans">
        <div className="flex items-center gap-4 mb-14 px-2">
          ...
          <div><h2 className="font-black text-2xl uppercase tracking-tighter leading-none">PIRU</h2></div>
        </div>
        <nav className="flex-1 space-y-3 font-sans">
          ...
          {user.role.split(' ')[0] === 'admin' && (<button onClick={() => setActiveTab('users')} className={`w-full flex items-center gap-4 p-5 rounded-3xl font-black text-xs uppercase transition-all ${activeTab === 'users' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-400 hover:bg-slate-50'}`}><Users size={20}/> Data Pegawai</button>)}
        </nav>
      ...
      <main className="flex-1 p-10 overflow-y-auto font-sans">
        <header className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-8 mb-10">
          <div className="flex-1 max-w-md">
            <h1 className="text-4xl font-black text-slate-900 tracking-tighter uppercase leading-none break-words">{user.name}</h1>
          ...
          <div className="flex flex-wrap items-center gap-3 xl:justify-end">
             {['admin', 'pimpinan', 'ketua'].includes(user.role.split(' ')[0]) && activeTab === 'laporan' && (
                <select className="p-3 bg-white border border-slate-200 rounded-xl font-black text-[10px] text-slate-600 shadow-sm outline-none" value={filterStaffName} onChange={e => setFilterStaffName(e.target.value)}>
                  ...
                  {users.filter(u => !['admin','pimpinan'].includes(u.role.split(' ')[0])).map(u => <option key={u.firestoreId} value={u.name}>{u.name}</option>)}
                </select>
              )}
            ...
            <button onClick={exportToExcel} className="bg-green-600 text-white px-4 py-3 rounded-xl font-black uppercase text-[10px] flex items-center gap-2 shadow-md"><Download size={14}/> Cetak</button>
            <button onClick={() => { resetReportForm(); setShowReportModal(true); }} className="bg-indigo-600 text-white px-6 py-3 rounded-xl font-black uppercase text-[10px] shadow-lg flex items-center gap-2"><Plus size={14}/> Entri</button>
          </div>
        </header>

        {activeTab === 'laporan' && (
          <div className="bg-white rounded-[3.5rem] shadow-sm border p-10 space-y-8">
            <div className="overflow-x-auto text-sm">
              <table className="w-full text-left">
                <thead className="bg-slate-50 border-b text-[10px] font-black text-slate-400 uppercase tracking-widest">
                  <tr><th className="p-8">Kegiatan</th><th className="p-8 text-center">Volume</th><th className="p-8 text-center">Cap %</th><th className="p-8 text-center">Ketua</th><th className="p-8 text-center">Pimp</th><th className="p-8 text-center">Aksi</th></tr>
                </thead>
                <tbody className="divide-y divide-slate-50">
                  {currentFilteredReports.map(r => (
                    <tr key={r.id} className="hover:bg-slate-50/50 transition-all">
                      ...
                      <td className="p-8 text-center font-black">{r.realisasi} / {r.target}</td>
                      ...
                      <td className="p-8 text-center">
                        <div className="flex justify-center gap-2">
                          ...
                          {/* TOMBOL NILAI: PASTI MUNCUL KARENA ROLE SUDAH DIBERSIHKAN */}
                          {['ketua', 'admin'].includes(user.role.split(' ')[0]) && r.userId !== user.username && (
                            <button onClick={() => submitGrade(r.id, 'ketua')} className="bg-amber-400 text-white px-5 py-2.5 rounded-2xl text-[10px] font-black uppercase shadow-md active:scale-95 transition-all">Ketua</button>
                          )}
                          
                          {['pimpinan', 'admin'].includes(user.role.split(' ')[0]) && r.userId !== user.username && (
                            <button onClick={() => submitGrade(r.id, 'pimpinan')} className="bg-indigo-600 text-white px-5 py-2.5 rounded-2xl text-[10px] font-black uppercase shadow-md active:scale-95 transition-all">Pimpinan</button>
                          )}
                        </div>
                      </td>
                    </tr>
        ...
        {activeTab === 'users' && (
           <div className="bg-white rounded-[3rem] shadow-sm border overflow-hidden p-10 font-sans">
              <div className="flex justify-between items-center mb-10">
                 <h2 className="text-2xl font-black uppercase tracking-tighter">Daftar Pegawai</h2>
                 <button onClick={() => setShowUserModal(true)} className="bg-indigo-600 text-white px-6 py-3 rounded-xl font-black uppercase text-[10px] shadow-lg flex items-center gap-2"><UserPlus size={14}/> Tambah Pegawai</button>
              </div>
              <table className="w-full text-left font-sans">
                 <thead><tr className="bg-slate-50 border-b text-[10px] font-black text-slate-400 uppercase tracking-widest"><th className="p-8">Nama</th><th className="p-8">Username</th><th className="p-8">Role</th><th className="p-8 text-center">Aksi</th></tr></thead>
                 <tbody>{users.map(u => (<tr key={u.firestoreId} className="border-b hover:bg-slate-50"><td className="p-8 font-black text-slate-800 uppercase">{u.name}</td><td className="p-8 font-bold text-indigo-500">@{u.username}</td><td className="p-8 font-bold text-slate-500 text-xs uppercase">{u.role}</td><td className="p-8 text-center"><button onClick={() => deleteDoc(doc(db, "users", u.firestoreId))} className="p-4 bg-red-50 text-red-400 rounded-2xl hover:bg-red-500 hover:text-white transition-all shadow-sm"><Trash2 size={20}/></button></td></tr>))}</tbody>
              </table>
           </div>
        )}
      </main>

      {/* MODAL USER - SUDAH SAYA BERSIHKAN DARI KATA ITALIC */}
      {showUserModal && (
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-xl flex items-center justify-center p-4 z-50 font-sans">
          <form onSubmit={handleAddUser} className="bg-white w-full max-w-xl rounded-[3.5rem] p-16 shadow-2xl relative">
            ...
            <h3 className="text-3xl font-black uppercase tracking-tighter mb-10 text-slate-800">Tambah Akun Pegawai</h3>
            <div className="space-y-5">
                ...
                <div className="grid grid-cols-2 gap-5">
                    ...
                    <input required type="password" placeholder="Password" className="w-full p-5 bg-slate-50 border rounded-2xl outline-none font-bold" onChange={e => setNewUser({...newUser, password: e.target.value})} />
                </div>
                ...
                {/* PERBAIKAN: value sekarang murni, sistem pasti baca role-nya */}
                ...
                <button type="submit" className="w-full bg-indigo-600 text-white font-black py-7 rounded-[2.5rem] shadow-xl uppercase tracking-widest text-[10px] mt-6 transition-all active:scale-95">Simpan Akun</button>
            </div>
          </form>
        </div>
      )}
      ...
      {showReportModal && (
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-xl flex items-center justify-center p-4 z-50 font-sans">
          <form onSubmit={handleSubmitReport} className="bg-white w-full max-w-2xl rounded-[3.5rem] p-16 shadow-2xl relative">
            ...
            <h3 className="text-4xl font-black uppercase tracking-tighter mb-10 text-slate-800">{isEditing ? "Update Laporan" : "Entri Laporan"}</h3>
            <div className="space-y-6">
               {['pimpinan', 'admin'].includes(user.role.split(' ')[0]) && !isEditing && (
               ...
               <input required type="text" placeholder="Uraian Pekerjaan" className="w-full p-6 bg-slate-50 rounded-3xl outline-none font-black text-slate-800 border border-slate-100" value={newReport.title} onChange={e => setNewReport({...newReport, title: e.target.value})} />
               ...
               <input className="w-full p-6 bg-slate-50 rounded-3xl outline-none font-black text-slate-800 border border-slate-100" placeholder="Satuan (Dokumen/Kegiatan)" value={newReport.satuan} onChange={e => setNewReport({...newReport, satuan: e.target.value})} />
            ...
            <button type="submit" className="w-full bg-indigo-600 text-white font-black py-7 rounded-[2.5rem] shadow-2xl uppercase tracking-widest text-xs mt-10 transition-all active:scale-95">Simpan Data</button>
          </form>
        </div>
      )}
    </div>
