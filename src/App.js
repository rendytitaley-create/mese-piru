    ...
    <div className="min-h-screen bg-slate-50 flex font-sans overflow-hidden text-slate-800 italic">
      <div className="w-72 bg-white border-r p-8 flex flex-col hidden md:flex font-sans not-italic">
        ...
        <nav className="flex-1 space-y-3 font-sans not-italic">
          ...
          {user.role.toLowerCase().includes('admin') && (<button onClick={() => setActiveTab('users')} className={`w-full flex items-center gap-4 p-5 rounded-3xl font-black text-xs uppercase transition-all ${activeTab === 'users' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-400 hover:bg-slate-50'}`}><Users size={20}/> Data Pegawai</button>)}
        </nav>
      ...
      {showReportModal && (
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-xl flex items-center justify-center p-4 z-50 font-sans italic">
          <form onSubmit={handleSubmitReport} className="bg-white w-full max-w-2xl rounded-[3.5rem] p-16 shadow-2xl relative italic">
            ...
            <h3 className="text-4xl font-black uppercase tracking-tighter mb-10 text-slate-800 italic">Entri Laporan</h3>
            <div className="space-y-6 italic">
               ...
               <input className="w-full p-6 bg-slate-50 rounded-3xl outline-none font-black text-slate-800 border border-slate-100 italic" placeholder="Satuan" value={newReport.satuan} onChange={e => setNewReport({...newReport, satuan: e.target.value})} />
