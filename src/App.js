...
const PIRUApp = () => {
  ...
  if (!user) return (
    <div className="h-screen bg-slate-900 flex items-center justify-center p-4 italic">
      <div className="bg-white w-full max-w-md rounded-[2.5rem] p-12 shadow-2xl text-center font-sans">
        ...
        <p className="text-[10px] font-black text-indigo-600 uppercase tracking-widest mb-1 leading-none italic">BPS KABUPATEN SERAM BAGIAN BARAT</p>
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex font-sans overflow-hidden text-slate-800 italic">
        ...
        {activeTab === 'users' ? (
          <div className="bg-white rounded-[3rem] shadow-sm border overflow-hidden p-10 font-sans italic">
            <table className="w-full text-left font-sans italic">
              <thead><tr className="bg-slate-50 border-b text-[10px] font-black text-slate-400 uppercase tracking-widest italic"><th className="p-8">Identitas Pegawai</th><th className="p-8">Role</th><th className="p-8 text-center">Aksi</th></tr></thead>
              <tbody>{users.map(u => (<tr key={u.firestoreId} className="border-b hover:bg-slate-50 transition-colors font-sans italic"><td className="p-8"><p className="font-black text-slate-800 uppercase text-lg tracking-tighter italic">{u.name}</p></td><td className="p-8 font-bold text-slate-500 text-xs uppercase italic">{u.role}</td><td className="p-8 text-center"><button onClick={() => deleteDoc(doc(db, "users", u.firestoreId))} className="p-4 bg-red-50 text-red-400 rounded-2xl hover:bg-red-500 hover:text-white transition-all shadow-sm"><Trash2 size={20}/></button></td></tr>))}</tbody>
            </table>
          </div>
        ) : (
      ...
      {/* MODAL USER - SUDAH SAYA PERBAIKI POKOK PERMASALAHANNYA DI SINI */}
      {showUserModal && (
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-xl flex items-center justify-center p-4 z-50 font-sans italic">
          <form onSubmit={handleAddUser} className="bg-white w-full max-w-xl rounded-[3.5rem] p-16 shadow-2xl relative italic">
            ...
            <div className="space-y-5 italic">
                ...
                {/* PERBAIKAN: Menghapus kata " italic" dari value agar role terbaca sistem */}
