...
const PIRUApp = () => {
  ...
  const dashboardStats = useMemo(() => {
    ...
    // FUNGSI UNTUK MENENTUKAN STATUS PROGRESS
    const getStatusText = (userReports) => {
      const total = userReports.length;
      if (total === 0) return "Belum Lapor";
      ...
      if (selesai === total) return "Selesai";
      if (progress > 0 || selesai > 0) return "Progres Penilaian";
      return "Pending (Ketua Tim)";
    };

    // REKAP SELURUH PEGAWAI (UNTUK ADMIN/PIMPINAN)
    const staffSummary = users.filter(u => u.role !== 'admin' && u.role !== 'pimpinan').map(s => {
      ...
      return { name: s.name, total, nilaiAkhir: ((avgCap + avgPimp) / 2).toFixed(2), status: getStatusText(sReports) };
    });
    ...
    return { 
      ...
      myStatus: getStatusText(myReports)
    };
        ...
        {activeTab === 'dashboard' ? (
          <div className="space-y-8 animate-in fade-in duration-500">
                {(user.role === 'pimpinan' || user.role === 'admin') ? (
                            ...
                            {dashboardStats.staffSummary.map((s, i) => (
                                <div key={i} className="p-7 bg-slate-50 rounded-[2.5rem] border border-slate-100 flex flex-col justify-between hover:border-indigo-200 transition-all">
                                    <div className="mb-4">
                                        ...
                                        <span className={`text-[9px] font-black uppercase px-2 py-0.5 rounded-lg ${s.status === 'Selesai' ? 'bg-green-100 text-green-600' : s.status === 'Progres Penilaian' ? 'bg-amber-100 text-amber-600' : 'bg-slate-200 text-slate-500'}`}>
                                            {s.status}
                                        </span>
                                    </div>
                    ...
                    // DASHBOARD PEGAWAI / KETUA TIM: DENGAN STATUS KONTROL PENILAIAN
                    <div className="space-y-8 animate-in fade-in duration-500">
                      ...
                      <div className="bg-indigo-900 rounded-[3rem] p-10 text-white flex flex-col md:flex-row items-center justify-between gap-8 shadow-2xl">
                          <div className="space-y-2">
                              <h2 className="text-3xl font-black uppercase tracking-tighter leading-none">Status Kontrol Penilaian</h2>
                              <p className="text-indigo-200">Bulan {selectedMonth}, Anda sedang berada pada tahap:</p>
                          </div>
                          <div className="bg-white/10 p-8 rounded-[2rem] text-center min-w-[280px] border border-white/10">
                              <p className="text-[10px] font-black uppercase tracking-widest opacity-60 mb-2 font-sans">Tahapan Saat Ini</p>
          ...
          <div className="bg-white rounded-[3rem] shadow-sm border border-slate-100 overflow-hidden p-8 space-y-6">
            <div className="flex flex-col md:flex-row justify-between gap-4">
                ...
                {(user.role === 'admin' || user.role === 'pimpinan' || user.role === 'ketua') && (
                    <select className="p-3 bg-slate-50 border-none rounded-xl font-bold text-xs outline-none" value={filterStaffName} onChange={e => setFilterStaffName(e.target.value)}>
      ...
      {showReportModal && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md flex items-center justify-center p-4 z-50 text-slate-800">
          <form onSubmit={handleSubmitReport} className="bg-white w-full max-w-2xl rounded-[3.5rem] p-12 shadow-2xl space-y-6 animate-in zoom-in-95">
            ...
            <button type="button" onClick={() => setShowReportModal(false)} className="w-full text-slate-400 font-bold uppercase text-[10px]">Batal</button>
          </form>
        </div>
      )}
