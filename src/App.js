      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden font-sans italic relative">
        <header className="p-10 pb-4 flex flex-col xl:flex-row justify-between items-start xl:items-center gap-8 italic sticky top-0 bg-slate-50 z-20">
          ...
          <div className="flex flex-wrap items-center gap-3 not-italic xl:justify-end">
             <button onClick={() => {localStorage.clear(); window.location.reload();}} className="md:hidden p-2 text-red-500"><LogOut size={24}/></button>
             {activeTab === 'penilaian' && (
                <>
        ...
        <div className="flex-1 overflow-y-auto p-10 pt-0 custom-scrollbar mb-20 md:mb-0">
          {activeTab === 'dashboard' && (
          ...
          {(activeTab === 'laporan' || activeTab === 'penilaian') && (
            <div className="italic">
              {/* DESKTOP VIEW */}
              <div className="hidden md:block bg-white rounded-[2.5rem] shadow-sm border p-6 space-y-8 italic mb-10 overflow-hidden">
              ...
              <div className="md:hidden space-y-4 mb-20">
                {currentFilteredReports.map((r, idx) => (
                  <div key={r.id} className="bg-white p-6 rounded-[2.5rem] shadow-sm border italic">
                    <div className="flex justify-between items-start mb-2"><span className="text-[10px] font-black text-indigo-600 italic">#{idx+1} - {r.satuan}</span><div className="flex gap-2">{activeTab === 'laporan' && r.status === 'pending' && <><button onClick={() => { setIsEditing(true); setCurrentReportId(r.id); setNewReport({title: r.title, target: r.target, realisasi: r.realisasi, satuan: r.satuan, keterangan: r.keterangan || ''}); setShowReportModal(true); }} className="text-indigo-600"><Edit3 size={16}/></button><button onClick={() => deleteDoc(doc(db, "reports", r.id))} className="text-red-400"><Trash2 size={16}/></button></>}</div></div>
                    <h4 className="font-black text-sm uppercase italic mb-4">{r.title}</h4>
                    <div className="grid grid-cols-2 gap-4 border-t pt-4">
                      ...
                      <div><p className="text-[8px] text-slate-400 uppercase font-black italic">Ketua/Pimp</p><p className="font-black text-xs italic text-indigo-600">{r.nilaiKetua} / {r.nilaiPimpinan}</p></div>
                    </div>
                    {activeTab === 'penilaian' && <div className="flex gap-2 mt-4"><button onClick={() => submitGrade(r.id, 'ketua')} className="flex-1 py-3 bg-amber-400 text-white rounded-xl text-[10px] font-black uppercase italic">Nilai Ketua</button><button onClick={() => submitGrade(r.id, 'pimpinan')} className="flex-1 py-3 bg-indigo-600 text-white rounded-xl text-[10px] font-black uppercase italic">Nilai Pimp</button></div>}
                  </div>
                ))}
              </div>
            </div>
      ...
      {showReportModal && (
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-xl flex items-center justify-center p-4 z-50 font-sans italic">
          <form onSubmit={handleSubmitReport} className="bg-white w-full max-w-2xl rounded-[3.5rem] p-12 shadow-2xl relative italic">
            <button type="button" onClick={() => { resetReportForm(); setShowReportModal(false); }} className="absolute top-8 right-8 p-4 bg-slate-50 rounded-full text-slate-400 italic"><X size={20}/></button>
            <h3 className="text-3xl font-black uppercase tracking-tighter mb-8 text-slate-800 italic">{isEditing ? "Update Pekerjaan" : (activeTab === 'penilaian' ? "Entri Anggota" : "Entri Pekerjaan")}</h3>
        ...
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-xl flex items-center justify-center p-4 z-50 font-sans italic">
          <form onSubmit={handleAddOrEditUser} className="bg-white w-full max-w-xl rounded-[3rem] p-12 shadow-2xl relative italic">
            ...
            <div className="space-y-4 italic">
                ...
                <button type="submit" className="w-full bg-indigo-600 text-white font-black py-6 rounded-2xl shadow-xl uppercase tracking-widest text-[10px] mt-6 italic">Simpan Pegawai</button>
            </div>
          </form>
        </div>
      )}
