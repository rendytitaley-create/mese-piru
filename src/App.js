    ...
    <div className="h-screen bg-indigo-900 flex items-center justify-center p-4">
      <div className="bg-white w-full max-w-md rounded-[3rem] p-10 shadow-2xl">
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          ...
          <input type="text" placeholder="Username" className="w-full p-4 bg-slate-100 rounded-2xl outline-none font-bold" onChange={e => setAuthForm({...authForm, username: e.target.value})} />
          <input type="password" placeholder="Password" className="w-full p-4 bg-slate-100 rounded-2xl outline-none font-bold" onChange={e => setAuthForm({...authForm, password: e.target.value})} />
    ...
    <div className="min-h-screen bg-slate-50 flex font-sans overflow-hidden">
      ...
      <main className="flex-1 p-10 overflow-y-auto">
        <header className="flex justify-between items-center mb-10">
          <div>
            <h1 className="text-3xl font-black text-slate-800 tracking-tighter uppercase">HALO, {user.name}</h1>
          ...
          <div className="flex items-center gap-4">
            ...
            {/* Hanya Pegawai & Ketua Tim yang bisa Lapor (Admin/Pimpinan tidak) */}
            {(user.role === 'pegawai' || user.role === 'ketua') && activeTab === 'laporan' && (
        ...
        {activeTab === 'users' && user.role === 'admin' ? (
            <div className="bg-white rounded-[2.5rem] shadow-sm border overflow-hidden">
                <table className="w-full text-left">
                    <thead><tr className="bg-slate-50 border-b text-[10px] font-black text-slate-400 uppercase"><th className="p-6">Pegawai</th><th className="p-6">Jabatan</th><th className="p-6 text-center">Aksi</th></tr></thead>
          ...
          <div className="bg-white rounded-[3rem] shadow-sm border border-slate-100 overflow-hidden">
                ...
                <tbody className="divide-y divide-slate-50">
                  {filteredReports.map(r => (
                    <tr key={r.id} className="hover:bg-slate-50/50 transition-all">
                      ...
                      <td className="p-8 text-center font-black text-slate-700">{r.realisasi} / {r.target} <span className="text-[10px] block text-slate-400">{r.satuan}</span></td>
                      ...
                      <td className="p-8 text-center flex justify-center gap-2">
                         {/* LOGIKA EDIT/HAPUS PEGAWAI: HANYA JIKA STATUS PENDING */}
                         {user.role === 'pegawai' && r.status === 'pending' && (
                            <>
                               <button onClick={() => handleEditClick(r)} className="text-indigo-600 p-2"><Edit3 size={18}/></button>
                               <button onClick={() => handleDeleteReport(r.id)} className="text-red-400 p-2"><Trash2 size={18}/></button>
                            </>
                         )}
                         {/* LOGIKA NILAI KETUA TIM */}
                         {user.role === 'ketua' && r.status === 'pending' && (
                            <button onClick={() => submitGrade(r.id, 'ketua')} className="bg-amber-500 text-white px-4 py-2 rounded-xl text-[10px] font-black uppercase">Nilai Ketua</button>
                         )}
                         {/* LOGIKA NILAI PIMPINAN */}
                         {user.role === 'pimpinan' && r.status === 'dinilai_ketua' && (
                            <button onClick={() => submitGrade(r.id, 'pimpinan')} className="bg-indigo-600 text-white px-4 py-2 rounded-xl text-[10px] font-black uppercase">Nilai Pimpinan</button>
                         )}
                         {(r.status === 'selesai' || (user.role === 'pegawai' && r.status !== 'pending')) && <CheckCircle2 className="text-green-500" size={20}/>}
                      </td>
                    </tr>
                  ))}
                </tbody>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 text-center">
                <div className="bg-white p-10 rounded-[3rem] shadow-sm border"><p className="text-slate-400 text-[10px] font-black uppercase mb-3">Kegiatan</p><p className="text-6xl font-black">{filteredReports.length}</p></div>
                <div className="bg-white p-10 rounded-[3rem] shadow-sm border text-green-600"><p className="text-slate-400 text-[10px] font-black uppercase mb-3">Selesai</p><p className="text-6xl font-black">{filteredReports.filter(r => r.status === 'selesai').length}</p></div>
                <div className="bg-white p-10 rounded-[3rem] shadow-sm border text-amber-500"><p className="text-slate-400 text-[10px] font-black uppercase mb-3">Avg Nilai</p><p className="text-6xl font-black">{(filteredReports.reduce((a, b) => a + (b.nilaiPimpinan || 0), 0) / (filteredReports.length || 1)).toFixed(1)}</p></div>
            </div>
        )}
      </main>

      {/* MODAL LAPORAN (BISA EDIT) */}
      {showReportModal && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md flex items-center justify-center p-4 z-50">
          <form onSubmit={handleSubmitReport} className="bg-white w-full max-w-2xl rounded-[3.5rem] p-12 shadow-2xl space-y-6">
            <h3 className="text-3xl font-black uppercase">{isEditing ? "Edit Laporan" : "Entri Kinerja"}</h3>
            <div className="space-y-4">
               ...
               <textarea className="w-full p-5 bg-slate-50 rounded-2xl outline-none font-black h-24" placeholder="Keterangan" value={newReport.keterangan} onChange={e => setNewReport({...newReport, keterangan: e.target.value})} />
            </div>
            <button type="submit" className="w-full bg-indigo-600 text-white font-black py-6 rounded-[2rem] uppercase shadow-xl">{isEditing ? "Simpan Perubahan" : "Kirim Laporan"}</button>
            <button type="button" onClick={() => setShowReportModal(false)} className="w-full text-slate-400 font-bold">Batal</button>
          </form>
        </div>
      )}
      ...
      {showUserModal && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md flex items-center justify-center p-4 z-50">
          <form onSubmit={(e) => {
            ...
            <h3 className="text-2xl font-black uppercase">Tambah User Baru</h3>
            ...
            <button type="submit" className="w-full bg-green-600 text-white font-black py-5 rounded-[2rem] shadow-xl">Simpan User</button>
