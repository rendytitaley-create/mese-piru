...
const PIRUApp = () => {
  ...
  const handleAddUser = async (e) => {
    e.preventDefault();
    try {
      // Membersihkan role dari teks 'italic' jika terbawa
      const cleanRole = newUser.role.split(' ')[0];
  ...
  const handleSubmitReport = async (e) => {
    ...
    try {
      if (isEditing && currentReportId) {
        ...
        if (['pimpinan', 'admin'].includes(user.role.split(' ')[0]) && newReport.targetUser) {
  ...
  const currentFilteredReports = useMemo(() => {
    ...
    const roleCheck = user?.role?.split(' ')[0];
    if (roleCheck === 'pegawai') res = res.filter(r => r.userId === user.username);
    if (['pimpinan', 'admin', 'ketua'].includes(roleCheck) && filterStaffName !== 'Semua') {
          ...
          <div className="flex flex-wrap items-center gap-3 not-italic xl:justify-end">
             {['admin', 'pimpinan', 'ketua'].includes(user.role.split(' ')[0]) && activeTab === 'laporan' && (
                <select className="p-3 bg-white border border-slate-200 rounded-xl font-black text-[10px] text-slate-600 shadow-sm outline-none" value={filterStaffName} onChange={e => setFilterStaffName(e.target.value)}>
                  ...
                  {users.filter(u => u.role.split(' ')[0] !== 'admin' && u.role.split(' ')[0] !== 'pimpinan').map(u => <option key={u.firestoreId} value={u.name}>{u.name}</option>)}
                </select>
              )}
            ...
            {user.role.split(' ')[0] === 'admin' && activeTab === 'users' && (<button onClick={() => setShowUserModal(true)} className="bg-indigo-600 text-white px-4 py-3 rounded-xl font-black uppercase text-[10px] flex items-center gap-2 shadow-md italic"><UserPlus size={14}/> Tambah Akun</button>)}
        ...
        {activeTab === 'users' ? (
          <div className="bg-white rounded-[3rem] shadow-sm border overflow-hidden p-10 font-sans italic">
            <table className="w-full text-left font-sans italic">
              <thead><tr className="bg-slate-50 border-b text-[10px] font-black text-slate-400 uppercase tracking-widest italic"><th className="p-8 italic">Identitas Pegawai</th><th className="p-8 italic">Role</th><th className="p-8 text-center italic">Aksi</th></tr></thead>
              <tbody>{users.map(u => (<tr key={u.firestoreId} className="border-b hover:bg-slate-50 transition-colors font-sans italic"><td className="p-8 italic"><p className="font-black text-slate-800 uppercase text-lg tracking-tighter italic">{u.name}</p></td><td className="p-8 font-bold text-slate-500 text-xs uppercase italic">{u.role}</td><td className="p-8 text-center italic"><button onClick={() => deleteDoc(doc(db, "users", u.firestoreId))} className="p-4 bg-red-50 text-red-400 rounded-2xl hover:bg-red-500 hover:text-white transition-all shadow-sm italic"><Trash2 size={20}/></button></td></tr>))}</tbody>
            </table>
          </div>
        ) : (
      ...
      {showUserModal && (
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-xl flex items-center justify-center p-4 z-50 font-sans italic">
          <form onSubmit={handleAddUser} className="bg-white w-full max-w-xl rounded-[3.5rem] p-16 shadow-2xl relative italic">
            ...
            <h3 className="text-3xl font-black uppercase tracking-tighter mb-10 text-slate-800 italic">Tambah Akun</h3>
