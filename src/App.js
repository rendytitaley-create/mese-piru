  ...
  const handleSubmitReport = async (e) => {
    ...
    try {
      if (isEditing && currentReportId) {
        ...
        // FITUR PENILAIAN SEKALIGUS (Konteks Tab Penilaian)
        ...
        <div className="flex-1 overflow-y-auto p-10 pt-0 custom-scrollbar">
              ...
              {['admin', 'pimpinan'].includes(user.role) ? (
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 mb-10">
                  {dashboardStats.staffSummary.map((s, i) => (
                    <div key={i} className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm italic group hover:border-indigo-200 transition-all">
                      <div className="flex justify-between items-start mb-4">
                        <div className="bg-slate-50 p-3 rounded-2xl group-hover:bg-indigo-50 transition-colors"><Users size={20} className="text-indigo-600"/></div>
                      ...
                      <p className="font-black text-lg text-slate-800 uppercase tracking-tighter leading-none mb-1 italic">{s.name}</p>
                      <p className="text-[10px] text-slate-400 font-bold uppercase mb-4 italic">{s.detailCount} Laporan</p>
                      <div className="flex justify-between items-end border-t pt-4">
                        <p className="text-[9px] font-black text-slate-400 uppercase italic">Estimasi Nilai</p>
          ...
          {activeTab === 'users' && (
            <div className="bg-white rounded-[2.5rem] shadow-sm border overflow-hidden p-6 font-sans italic mb-10">
              ...
              <button onClick={() => setShowUserModal(true)} className="mt-4 bg-indigo-600 text-white px-6 py-3 rounded-2xl font-black uppercase text-[10px] flex items-center gap-2 italic"><UserPlus size={14}/> Tambah Pegawai</button>
            </div>
          )}
